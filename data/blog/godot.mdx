...
Godot is a free, open source videogame engine. It uses nodes as building blocks and scenes for grouping them. For scripting you can use GDScript or C#. To learn more you can visit their site, Facebook, twitter, reddit or GitHub.
...
When you open Godot you encounter the *Project Administrator*, use it to edit, play, search, create, import, rename or delete your projects. The templates section has example projects to quick start an idea. Every project needs a unique name, folder and selected backend (GLES2 or GLES3). To import a project use the search option and open the folder that contains a `project.godot` file.
...
- Playtest Buttons (Play project, pause, stop, play scene, play custom)
- A Toolbar that reacts to the selected workspace.
- Bottom Panel: Output, Debugger, Animation, Audio.

The *Viewport* is divided into 6 docks:

- Scene: Every node of the opened scene.
...
With the **Scene** menu you basically create, open, save, reload and close scenes (with `tscn` extension). With the **Project** menu you can modify the settings, select a version control or export the project. In the project settings `Aplication-Run-MainScene` we can select the main starting scene. With the **Editor** menu you can manage export templates, and control multiple aspects of the editor itself across projects.
...
A **Scene** is the hierarchical organization of nodes that create a tree of nodes:

1. A scene has a root node
2. Is is independent of other scenes via a unique file
3. Can be instanced (used inside another scene)

When we run the project what we are actually doing is instancing the root node of the main scene of the project, Godot can be described simply as a scene editor where we change the scenes by modifying the nodes that compose them. Note that instancing effectively allows us to use scenes inside other scenes treating them as nodes.

In the example proyect we have a Ball Scene and a Main scene. Ball is composed of a RigidBody2D node while Main contains obstacles made of StaticBody2D nodes. To instance the Ball scene inside Main we use the link button.

SHOW THE LINK BUTTON AND IMAGES

By default when modifying a scene every instance is updated, if we want to change the value only to the selected instance we indicate it with the option Make Unique, a gray return icon indicates that the general value is diferent from the one in the current instance.

By using a sistem of nodes and scenes we dont have to use design patterns like MVC or Entity Relationship and we can focus on terms of visual elements, it is therefore recomended to create a diagram of most scenes and nodes before starting a new ptoyect.


![Node Relationships](/static/public/images/godot/NodeRelations.jpg)

It is recomended that each element of the diagram has its own scene until it can be described using only nodes, it is perfectly normal to have scenes composed onlty of other scenes.

![Scene Relationships](/static/public/images/godot/SceneRelations.jpg)

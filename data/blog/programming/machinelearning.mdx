...
Get the data of an input tag (text) 
from html to my python app
as a variable
without form

Or solutions to:
Pass multiple variables
with an (a) tag with (url_for)

I cant get the data from the input tag with form
Since I use a dynamic URL:

<a href="{{ url_for('polling', Qid=question.name, option=1) }}">Vote</a

I want to use the data of input tag in my route:
...
def index():
    dataInput = request.get("Q1","")
    return render_template('home.html', data=dataInput)
...
<input type="text" id="1" name="Q1" value="Default Answer">

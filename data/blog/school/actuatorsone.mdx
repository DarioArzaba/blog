...
$$
\begin{gathered}
	\omega_m = \frac{- (R_a + R_{ae})}{(K_a\Phi)^2} (T) + \frac{V_t}{K_a \Phi}\\
	\omega_m =  - K_2(T) + K_1(V_t)
\end{gathered}
$$
...
In a series motor configuration, where the armature and field circuits are in series, if we try to control the speed of the motor using a rheostat then the field control and the armature resistance control methods will be the same. We use series configuration for large startup torques (subways, car gears, blenders) at low speeds and high speeds at low torques. assuming

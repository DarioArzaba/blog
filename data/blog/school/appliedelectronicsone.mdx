...
Virtual ground is a technique used to simplify the Op Amp analysis, we assume that the invertor input is connected to ground $V_n = V_p$ meaning that it has no potential differential, this is generally true even if we don't have a galvanic connection to ground. Note that this is only valid if the open loop gain is much more than the closed loop gain from the resistors.

Generally the open loop gain will be greater than the closed loop gain since it isn't common to use resistors with such a big difference between their values. Another thing to consider is that you should use resistors with high resistances to help the input current be as close to zero as possible.

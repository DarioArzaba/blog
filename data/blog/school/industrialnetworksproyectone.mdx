...
GRAFCET as an automation system abstraction doesn't know the specific system components, it simply knows that stages represent actions (operation) and transitions represent comparisons (control). GRAFCET stages must be elementary stages meaning that in order to  execute a complex operation (macrostage) you must subdivide its actions into its most elementary operations. Note that you may need to use as many SET-RESET instructions as there are state variables in your process.
...
The functional GRAFCET stage is the first stage of the automation system design, it is where we create the general succession of stages or actions and their transition conditions. The sensors and actuators GRAFCET stage is the second stage where we specify the sensors and actuators that the automata will use in each stage and transition. The last stage is the control system design stage that consists on translating the graph into ladder language.
...
The activation or latch of the internal bit of a stage N takes place when the previous stages are activated as every transition condition has been fulfilled. On the other hand the deactivation or unlatch of the internal bit of a stage occurs when the subsequent stage is activated. The combinational ladder consists of executing every action needed within each stage, a logical scheme is obtained detailing the states of every state variable and condition. 

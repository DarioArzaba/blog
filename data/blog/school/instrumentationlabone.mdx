...
* Resolution: Smallest *increment* between inputs after the threshold that it can detect with certainty.
...
* Time: Related to the instrument's response (output) when applying a step function (input).
	...
	* Second Order Response
		...
		* Overshoot: Difference between the initial peak and the settling (smaller than 2%) output value.
...
Power is an expression of how much energy is being used per unit of time. In an electric circuit, sensor or instrument we generally measure its apparent power as $P_S = VI$ in Volts per Ampere. When the circuit uses DC or AC but only has a resistive load (phase angle equals 0) then the apparent power will be equal to the active (real, true) power used since $P_S = VI = P_P = VI\cos(0)$ in Watts. 

When the circuit uses an AC source but has an inductive or a capacitive load then there will be a phase angle between the voltage and current supplied to the load. This phase angle or difference is caused by the residual energy stored in the inductors as magnetic fields or the capacitors as electric field, by measuring this phase angle we can calculate the power factor of the circuit:
...
When there is phase angle present the reactive power will be the unusable energy lost in the inductors or capacitors of the circuit $P_Q = VI\sin(\theta)$. Reactive power can be present when working with inductive motors, to mitigate the parasitic current that remains after it rotates we introduce additional capacitors reducing the overall power factor of the system. 

Note that we can define apparent power in terms of the active and reactive powers $P_S = \sqrt{P_P^2 + P_Q^2}$. The phase angle of an ideal capacitive circuit (negative reactive power) makes the current to lead the supplied voltage by 90° and in an ideal inductive circuit (positive reactive power) it makes the current to lag the supplied voltage by 90°.
...
The formulas mean that the self inductance of the coil can vary only by changing the number of turns, the geometric form factor (G) of the coil or the permeability of the medium. Inductive proximity sensors have a high switching rate (targets in fast succession), high durability and high integration, however they can only sense metals and have a maximum detection range of 100mm. 

Capacitive proximity sensors work by generating an electric field through a plate and then use an oscillator (RC circuit) to generate a sinusoidal signal. As the target passes in front of the sensor it acts as the second plate needed to create a capacitor therefore increasing the capacitance of the electric field. 

They have high array of material detection, work even when completely insulated (robust), have adjustable sensitivity and no moving parts, however they have a higher cost and a maximum detection range of 25mm. Their detection range will depend on the active surface of the plate, the target's dielectric constant and the surface area of the target. Temperature and target's speed also have a minimal effect.
...
They are transformers manufactured to have multiple output terminals, each with a different number of coils. They are used inside variable voltage sources like the brand Variac. When working with high voltage circuits we are more likely to need power resistors that can dissipated the extra energy, generally high power resistors are expected to be mounted on a sheet of metal for additional heat dissipation.

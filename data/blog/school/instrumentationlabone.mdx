...
* Null: In some phenomena it is convenient to measure the opposite effect needed to maintain a pointer stationary and then infer the value of the original quantity. For example a deadweight pressure gauge scale.
...
Dynamic: Measured quantities that change rapidly relative to time. For example when measuring force values because opposite force reactions introduce a lot of noise.

Dynamic relation between input and output.

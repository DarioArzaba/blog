...
| `ping <host>` | Sends data packages to a host and waits for the echo response. | Use hostname or IP |
...
| Command | Description | Switches |
| :---: | :---: | :---:|
| `enable` | Enables privileged EXEC mode | None |
| `disable` | Exit privileged EXEC mode | None |
| `configure <device>` | Enter global configuration mode | Device can be a terminal |
| `line <device>` | Enter line configuration mode | Device can be a console followed by an ID |
| `interface <device>` | Enter interface or port configuration mode | Device can be a vlan port followed by an ID |
| `exit` | Exit configuration mode. Also to logout. | None |
| `hostname <string>` | Configure the device hostname | None |
| `password <string>` | Define a user password | None |
| `enable secret <string>` | Define a privileged user password | 
| `login` | Login to the device profile using its password | 
| `line vty <firstline> <lastline>` | Define VTY line password | Usually 16 lines numbered 0 to 15 |
| `service password-encryption` | Encrypt defined passwords | None |
| `show running config` | Verify password encryption | None |
| `banner motd #<string>#` | Add a terminal login banner | None |
| `ip adress <ip> <subnetmask>` | Define the device IP and subnet mask | None |
| `no shutdown` | It enables the status of the configured interface | None | 

...
The Ethernet protocol works at both the physical and data link layers. It is a family of network technologies defined in 802.2 and 802.3 that create Ethernet data frames. Like other data link protocols it uses the LCC and MAC sublayers to work.

**Ethernet Frame**

The data encapsulation done by the MAC sublayer (802.3) when using Ethernet creates Ethernet frames (internal structure), Ethernet addressing (MAC source and destination) and Ethernet error detection (Frame Check Sequence FCS Trailer). Every Ethernet frame is expected to come from an Ethernet NIC into another Ethernet NIC inside the LAN.
...
When a device forwards data through Ethernet the Frame Header will include the source and destination MACs. When a NIC receives an Ethernet frame it examines its header to see if it matches its physical MAC address stored in RAM, after approval it sends the frame to the next OSI layers for decapsulation.
...
Each device on the Ethernet LAN receives and processes an Ethernet broadcast frame. The characteristics of an Ethernet transmission are as follows:
It has a destination MAC address of FF-FF-FF-FF-FF-FF in hexadecimal (48 units in binary).
All ports on the Ethernet switch are flooded except the incoming port. It is not forwarded by a router.
If the encapsulated data is an IPv4 broadcast packet, this means that the packet contains a destination IPv4 address that has all (1s) on the host side. This numbering in the address means that all hosts on that local network (broadcast domain) will receive and process the packet.
Ethernet MAC addresses Multicast MAC address
A group of devices belonging to the same multicast group receives and processes an Ethernet multicast frame.
There is a destination MAC address 01-00-5E when the encapsulated data is an IPv4 multicast packet and a destination MAC address of 33-33 when the encapsulated data is an IPv6 multicast packet.
There are other multicast destination MAC addresses reserved for when the encapsulated data is not IP, such as Spanning Tree Protocol (STP).
All ports on the Ethernet switch are flooded except the incoming port, unless the switch is configured for multicast snooping. It is not forwarded by a router, unless the router is configured to route multicast packets.
Because multicast addresses represent a group of addresses (sometimes referred to as a “host group”), they can only be used as the destination for a packet. The source always has a unicast address.
As with unicast and broadcast addresses, the multicast IP address requires a corresponding multicast MAC address


**MAC Address Table**

Every Ethernet Switch has a MAC address table on their data link layer that it uses to forward Ethernet frames. The switch doesn't take into account the data or the data protocol (IPv4 ARP packet or IPv6 ND packet) just the Ethernet MAC addresses when forwarding data. Switches must examine their MAC table for every frame unlike Legacy Ethernet Hubs that forwarded the data on every available port except the incoming port. The table is empty when the switch is powered for the first time and it fills as new devices are connected to it. It is also called the Content Addressable Memory (CAM) table.



Examine source MAC address (learn)
Each frame entering a switch is reviewed for new information. This is done by examining the frame's source MAC address and the port number through which it entered the switch. If the source MAC address does not exist, it is added to the table, along with the input port number. If the source MAC address exists, the switch updates the update timer for that entry. By default, most Ethernet switches keep a table entry for five minutes.
...
The MAC Address Table Learning the Switch and Forwarding (cont.)
Find the destination MAC address (Forward)
If the destination MAC address is a unicast address, the switch looks for a match between the destination MAC address of the frame and an entry in the MAC address table. If the destination MAC address is in the table, it forwards the frame on the specified port. If the destination MAC address is not in the table, the switch forwards the frame on all ports except the incoming one. This is known as unicast.

Note: If the destination MAC address is broadcast or multicast, the frame is also sent to all ports except the incoming one.

The MAC Address Table Frame Filtering
As a switch receives frames from different devices, you can complete the MAC address table by examining the MAC address of each frame. When the switch MAC address table contains the destination MAC address, you can filter the frame and forward a single port.

The MAC address table MAC address tables on connected switches
This video will cover the following:
How Switches Create MAC Address Tables
How to forward frames based on the contents of your MAC address tables


**Switch Forwarding**

The speed and method to forward data depend on the specific devices. In Ethernet switches some methods include: Store and Forward Switching where it receives a frame and ensures its valid, if it passes the CRC check the switch looks at the destination MAC and determines the appropriate exit interface decreasing frame corruption and bandwidth reduction due to corrupt data. It can be used in QoS analysis for traffic priority frame classification.
...
In shared based memory ports share a common frame buffer, the memory of the buffers is dynamically allocated and the frames themselves are dynamically bound to their destination ports. This method allows for larger frame transmission with fewer abandoned frames which is important for asymmetric switching (different data rates for every port), meaning that some ports can have dedicated bandwidth priority.

When switches communicate their individual ports bandwidth (speed) and duplex setting (half or full) must match. The auto-negotiation feature allows for two switches to negotiate their best bandwidth and duplex configuration (Gigabit only work in full duplex). Duplex mismatch and crossover/straight cable mismatch are a common cause of performance problems on Ethernet links. Note that direct router host Ethernet connections must be done with crossover. Today most switches support automatic media dependent crossover or Auto-MDIX.
...
IP version 4 (IPv4) and IP version 6 (IPv6) are the main network layer communication protocols.
...
IP can use an IPv4 or IPv6 packet and not affect the Layer 4 segment.
...
Note: NAT will change the addressing, but it will be discussed in a later module.

Network layer characteristics IP characteristics

IP is intended to have low overhead and can be described as:
Without connection
Minimal service
...
IP Connectionless
IP does not establish any connection to the destination before sending the packet.
No control information is needed (synchronizations, confirmations, etc.).
The destination will receive the packet when it arrives, but no prior IP notifications are sent.
If there is a need for connection-oriented traffic, another protocol will handle this (usually TCP at the transport layer).

Network Layer Characteristics Best Effort

IP is the best effort
IP will not guarantee delivery of the package.
IP has reduced the overhead as there is no mechanism to forward data that is not received.
IP does not expect acknowledgments.
IP does not know if the other device is operational or if it received the packet.

Network layer characteristics Media Independence

IP is not reliable:
You cannot manage or fix undelivered or corrupted packages.
IP cannot retransmit after failure.
IP cannot realign stream packets.
IP must rely on other protocols for these functions.
IP is media independent:
IP does not refer to the type of frame required at the data link layer or the type of media at the physical layer.
IP can be sent over any type of medium: copper, fiber, or wireless.

Network Layer Characteristics Media Independence (cont.)

The network layer will set the Maximum Transmission Unit (MTU).
The network layer receives it from the control information sent by the data link layer.
The network then sets the MTU size.
Fragmentation is when Layer 3 divides the IPv4 packet into smaller units.
Chunking causes latency.
IPv6 does not fragment packets.
Example: The router goes from Ethernet to a slow WAN with a smaller MTU.

8.2 IPv4 packet

IPV4 packet IPV4 packet header

IPv4 is the main communication protocol for the network layer.
The network header has many purposes:
It ensures that the package is sent in the correct direction (to the destination).
Contains information for network layer processing in various fields.
The header information is used by all Layer 3 devices handling the packet

IPV4 packet IPV4 packet header fields

IPv4 Network Header Features:
It is in binary.
Contains various information fields
Diagram is read from left to right, 4 bytes per line
The two most important fields are origin and destination.
...
Version This will be for v4, unlike v6, a 4-bit field = 0100
Differentiated services Used for QoS: DiffServ - DS field or the former IntServ - ToS or Type of service
Header Checksum Detect IPv4 Header Corruption
Time to Live (TTL) Layer 3 Hop Count. When it becomes zero, the router will drop the packet.
Internet ID Next Level Protocol: ICMP, TCP, UDP, etc.
Source IPv4 address 32-bit source address
Destination IPV4 address 32-bit destination address

8.3 IPv6 packets
IPv6 packets IPv4 limitations
IPv4 has three main limitations:
IPv4 address depletion - We have basically run out of IPv4 addressing.
Lack of end-to-end connectivity: for IPv4 to survive this long time, private addresses and NAT were created. This put an end to direct communications with public speaking.
Increased network complexity: NAT was intended as a temporary solution and creates network problems as a side effect of manipulating the network headers that it addresses. NAT causes latency and troubleshooting issues.

IPv6 packets Introduction to IPv6
...
Larger address space: based on 128-bit address, not 32-bit
Improved packet handling - simplified header with fewer fields
Eliminates the need for NAT - Since there is a large amount of addressing, there is no need to use private addressing internally and map to a shared public address

IPv6 packets IPv4 packet header fields in the IPv6 packet header

The IPv6 header is simplified, but it is not smaller.
The header is set to 40 Bytes or octets in length.
Several IPv4 fields have been removed to improve performance.
Some IPv4 fields have been removed to improve performance:
Bookmark
...
Significant fields in the IPv4 header:
Function Description
Version This will be for v6, unlike v4, a 4-bit field = 0110
Traffic class Used for QoS: Equivalent to the DiffServ - DS field
Stream Label Informs the device to handle identical stream labels in the same way, 20-bit field
Payload length This 16-bit field indicates the length of the data portion or the payload of the IPv6 packet
Next header I.D.s next level protocol: ICMP, TCP, UDP, etc.
Hop Limit Replaces the Layer 3 hop count of the TTL field
Source IPv4 address 128-bit source address
Destination IPV4 address 128-bit destination address

IPV6 packets IPV6 packet header (Cont.)

The IPv6 packet can also contain extension headers (EH).
EH Headers Features:
provide optional network layer information
are optional
are placed between the IPv6 header and the payload
can be used for fragmentation, security, mobility support, etc.

Note: Unlike IPv4, routers do not fragment IPv6 packets.
8.4 How a host is routed

How a Host is Routed Host Forwarding Decision

Packages are always created at the source.
...
How a Host Is Routed Host Forwarding Decision (Cont.)

The source device determines whether the destination is local or remote
Determination method:
IPv4 - The source uses its own IP address and subnet mask, along with the destination IP address
IPv6: the source uses the network address and prefix advertised by the local router
Local traffic is disconnected from the host interface to be handled by an intermediary device.
Remote traffic is forwarded directly to the default gateway on the LAN.

How Default Gateway Hosts Are Routed

A Layer 3 switch or router can be a default gateway.
Characteristics of a default gateway (DGW):
It must have an IP address in the same range as the rest of the LAN.
It can accept data from the LAN and is capable of forwarding traffic outside the LAN.
It can be routed to other networks.
If a device does not have a default gateway or a bad default gateway, its traffic will not be able to leave the LAN.

How hosts are routed A host routes to the default gateway
The host will know the default gateway (DGW) statically or via DHCP on IPv4.
IPv6 sends the DGW through a request from a router (RS) or it can be configured manually.
 A DGW is a static route that will be a route of last resort in the routing table.
All devices on the LAN will need the roter DGW if they intend to send traffic remotely.

How a Host is Routed Host Routing Tables
On Windows, route print or netstat -r shows the PC routing table
...
Introduction to Routing Router IP Routing Table
There are three types of routes in a router's routing table:
Directly Connected - These routes are automatically added by the router, as long as the interface is up and has addressing.
Remote - These are the routes that the router does not have a direct connection to and can be learned:
Manually - with a static route
Dynamically - by using a routing protocol for routers to share their information with each other
Default route: forwards all traffic to a specific address when there is no match in the routing table

Introduction to Routing Static Routing
Static route features:
It must be configured manually.
It must be manually adjusted by the administrator when there is a change in the topology
Good for small, non-redundant networks
Often used in conjunction with a dynamic routing protocol to configure a default route
...
Search for new optimal routes when there is a topology change
Dynamic routing can also share default static routes with the other routers

Introduction to routing Introduction to an IPv4 routing table
The show ip route command displays the following route sources:
L - Directly connected local interface IP address
C - Directly connected network
Y - The static route was manually configured by an administrator
OR - OSPF
D - EIGRP
...
The arp —a command displays the ARP table on a Windows 10 computer.
...
In this example, your ISP has assigned a public network address of 172.16.0.0/22 ​​(10 host bits) to your headquarters that provides 1,022 host addresses.

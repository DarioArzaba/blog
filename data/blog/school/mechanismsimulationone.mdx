...
The position of any point P in a given space is the relation it has with respect to the origin or to another point. This relation or path can be represented by a vector, for example the vector that goes from the origin O to the point P: $\vec{R_{PO}}$. Note that the speed and rotation of a *point* will always be equal to a linear change in position.
...
Rotational velocity or acceleration is not defined for a point but in the case a joint it is defined as the change in angle from a predefined axis, therefore for any arbitrary point inside a link we could measure angular velocity or acceleration.

**Loop Closure Equation**

For every vector between two points there is an infinite number of equivalent loop closure equations that relate them. Every vector is inherently positive but when broken into its components then each component can be positive or negative. 

$$
\begin{gathered}
	P = \vec{R_{PO}} = R^x_{PO}\vec{i} + R^y_{PO}\vec{j} + R^z_{PO}\vec{k}\\
	P = (R_{PO})(\theta_{PO}) = \vec{R_1} + \vec{R_2} = \vec{R_3} + \vec{R_4} + \vec{R_5} ...
\end{gathered}
$$

We should try to use vector loops that correlate to known length measurements so that we can reduce the number of unknown variables in the mechanism. In a 2D plane we represent the angle of a vector using a trigonometric function, generally the cosine with respect to the positive X axis. Note that by using a trigonometric function we are adding the correct sign to the vector's magnitude.
...
\begin{gathered}
	...
	R_{BA} = R_2 \land R_{CB} = R_3 \land R_{CA} = R_1\\
	...
	(R_x)(e^{j\theta_x}) + (R_y)(e^{j\theta_y}) = (R_2)(e^{j\theta_2}) + (R_3)(e^{j\theta_3})\\
	e^{j\theta_x} = \cos(\theta_x) + \sin(\theta_x) \, j\\
	\text{Real}\\
	(R_x)(\cos(\theta_x)) + (R_y)(\cos(\theta_y)) = (R_2)(\cos(\theta_2)) + (R_3)(\cos(\theta_3))\\ 
	\text{Imaginary}\\
	(R_x)(\sin(\theta_x)) + (R_y)(\sin(\theta_y)) = (R_2)(\sin(\theta_2)) + (R_3)(\sin(\theta_3))\\
	\theta_x = 0 \land \theta_y = 90\\
	\text{First Equation}\\
	(R_x)(\cos(\theta_x)) = (R_2)(\cos(\theta_2)) + (R_3)(\cos(\theta_3))\\ 
	\text{Second Equation}\\
	(R_y)(\sin(\theta_y)) = (R_2)(\sin(\theta_2)) + (R_3)(\sin(\theta_3))\\
\end{gathered}
$$
